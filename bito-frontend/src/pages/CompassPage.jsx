...
/* ================================================================
   CompassPage — goal input, generation, preview, and list
   ================================================================ */
const CompassPage = () => {
  const { user } = useAuth();
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const res = await compassAPI.generate(goalText.trim());
  ...
  const startEditHabit = (index) => {
    ...
    setEditForm({ ...activeCompass.system.habits[index] });
  };
  ...
  const removeHabit = async (index) => {
    const updatedHabits = activeCompass.system.habits.filter((_, i) => i !== index);
  ...
  // ── Preview view ──
  if ((view === "preview" || view === "detail") && activeCompass) {
    const t = activeCompass;
    const sys = t.system;
    ...
    return (
      <div className="max-w-3xl mx-auto p-6 space-y-6">
        {/* Back */}
        <button
          onClick={() => { setView("list"); setactiveCompass(null); setEditingHabit(null); }}
        ...
        >
          <ArrowLeftIcon className="w-4 h-4" /> Back to compass
        </button>

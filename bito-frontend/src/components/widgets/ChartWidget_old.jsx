...
export const ChartWidget = memo(
  ({
    title,
    type = "bar",
    chartType = "completion", // New prop to specify what type of chart data to show
    data = [],
    color = "#3B82F6",
    breakpoint = "lg",
    availableColumns = 8,
    ...
    const prevDataRef = useRef();// Get real habit data based on chart type
    ...
    useEffect(() => {
      if (type === "bar") {
        ...
        // Mark animation as complete        setTimeout(() => {
          setIsAnimating(false);
        }, finalData.length * 100 + 500);
      } else {
    ...
    // Initialize animated data once on mount
    useEffect(() => {
      setAnimatedData(finalData);
      setIsAnimating(false);
    }, []); // Empty dependency array - only run once

    // Use separate effect for data updates with proper comparison
    const prevDataRef = useRef();
    useEffect(() => {
      if (prevDataRef.current !== finalData) {
        setAnimatedData(finalData);
    ...
    if (breakpoint === "xs") {
      ...
      const avg =
        finalData.length > 0 ? Math.round(total / finalData.length) : 0;
    ...
    const renderChart = () => {
      const commonProps = {
        data: chartConfig.chartType === "pie" ? pieData : finalData,
        margin:
          breakpoint === "sm"
      ...
      switch (chartConfig.chartType) {
        ...
        default: // bar chart
          return (
            <BarChart
              {...commonProps}
              data={type === "bar" ? animatedData : finalData}
            >
              {/* Minimal grid - only horizontal lines */}
              {chartConfig.showGrid && (
                <CartesianGrid
            ...
            <div className="flex justify-around pt-2 mt-2 border-t border-[var(--color-border-primary)] text-xs flex-shrink-0 font-outfit">
              ...
              <div className="text-center">
                ...
                >
                  {isAnimating
                    ? "..."
                    : Math.max(...finalData.map((item) => item.value))}
                </div>

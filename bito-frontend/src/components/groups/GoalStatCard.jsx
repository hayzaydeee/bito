...
const GoalStatCard = ({ teamGoalChallenge, onCreateChallenge }) => {
  ...
  if (!teamGoalChallenge) {
    return (
      <div
        className="flex-1 min-w-0 rounded-2xl border border-[var(--color-border-primary)]/20 bg-[var(--color-surface-elevated)] p-5 flex flex-col gap-1 cursor-pointer transition-colors hover:border-[var(--color-border-primary)]/40 relative overflow-hidden"
  ...
  return (
    <div className="flex-1 min-w-0 rounded-2xl border border-[var(--color-border-primary)]/20 bg-[var(--color-surface-elevated)] p-5 flex flex-col gap-2">
      <div className="flex items-center justify-between">
        <p className="text-xs text-[var(--color-text-tertiary)] font-spartan uppercase tracking-wide truncate pr-2">
          {teamGoalChallenge.title || "Team goal"}
        </p>
        <span className="text-base font-spartan font-semibold text-[var(--color-text-primary)] flex-shrink-0">
          {pct}%
        </span>
      </div>

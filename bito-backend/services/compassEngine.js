/**
 * CompassEngine — generates phased habit systems from goals using LLM.
 *
 * v2 scope: generate(goalText, userId) → phased preview data.
 *           refine(compass, userMessage) → delta patches + assistant reply.
 */
...
function getClient() {
  ...
  try {
    ...
    console.warn('[CompassEngine] Failed to create OpenAI client:', err.message);
    return null;
  }
  return _client;
}
...
const MODEL = process.env.COMPASS_LLM_MODEL || process.env.INSIGHTS_LLM_MODEL || 'gpt-4o-mini';
...
async function buildUserDossier(userId) {
  ...
  try {
    ...
    const habits = await Habit.find({ userId, isActive: true, isArchived: false })
      .select('name description category frequency weeklyTarget methodology target schedule stats activatedAt source compassId')
      .lean();
    ...
    if (habits.length > 0) {
      dossier.habits = habits.map(h => {
        ...
        if (h.source === 'compass') entry.fromCompass = true;
    ...
    console.warn('[CompassEngine] Dossier build partial failure:', err.message);
  }
...
function formatDossierForPrompt(dossier) {
  ...
  if (dossier.habits.length > 0) {
    ...
    for (const h of dossier.habits) {
      ...
      if (h.fromCompass) line += ' [from compass]';
      sections.push(line);
    }
  } else {
...
const SYSTEM_PROMPT = `You are Bito's Compass Engine — an expert life systems designer embedded within a habit tracking app.
...
6. INTERCONNECTED: Habits should support each other. A fitness compass should include recovery and nutrition, not just exercise.
...
const REFINE_SYSTEM_PROMPT = `You are Bito's Compass Engine — refining an existing phased habit system based on user feedback.
...
- In "active" mode: this compass has real habits created from it. Your patches will modify LIVE habits.
...
suiteGroups clusters related sub-goals that should become one compass together.
...
IMPORTANT: Create at most 5 suiteGroups. If there are many goals, aggressively group related ones together. For example, group all fitness goals, all creative goals, all financial goals, etc. Never create more than 5 groups.`,
    ...
    console.warn('[CompassEngine] Goal parse failed, using fallback:', err.message);
    return fallback;
  }
}

/**
 * Generate a phased compass system from a goal.
...
/**
 * Generate a single compass from a goal.
 * @private
 */
async function generateSingle(goalText, userId, parsed, dossier, dossierBlock, clarificationAnswers, suiteContext) {
  ...
  // Suite context — if this is one compass within a suite, inform the AI about siblings
  if (suiteContext) {
    promptParts.push(`\n--- SUITE CONTEXT ---`);
    promptParts.push(`This compass is part of a linked suite: "${suiteContext.suiteName}"`);
    promptParts.push(`Other compasses in this suite:`);
    ...
    promptParts.push(`Design this compass to complement — not duplicate — the siblings.`);
  ...
  const useWebSearch = process.env.COMPASS_WEB_SEARCH !== 'false' && !suiteContext;
  const createParams = {
    model: MODEL,
    temperature: 0.7,
    max_tokens: 3000,
  ...
  try {
    ...
    if (useWebSearch) {
      console.warn('[CompassEngine] LLM call with web search failed, retrying without:', wsErr.message);
  ...
  try {
    ...
    console.error('[CompassEngine] Failed to parse LLM response:', raw);
...
/**
 * Generate a suite of compasses from a compound goal.
 * Creates one compass per suiteGroup, linked by suiteId.
 * @private
 */
async function generateSuite(goalText, userId, parsed, dossier, dossierBlock, clarificationAnswers) {
  ...
  const tasks = parsed.suiteGroups.map((group, gi) => {
    ...
    // Suite context — tell the AI about sibling compasses
    ...
    const batchResults = await Promise.all(
      chunk.map(async (task) => {
        try {
          ...
          console.error(`[CompassEngine] Suite group "${task.group.name}" (${task.gi + 1}/${tasks.length}) failed:`, err.message, err.stack?.split('\n')[1]?.trim());
  ...
  if (results.length === 0) {
    throw new Error('Failed to generate compasses — all groups encountered errors. Try simplifying your goals or reducing the number of goals.');
  }
...
/**
 * Refine an existing compass based on user feedback.
 * Returns patches + assistant message (not saved to DB — caller saves).
 *
 * @param {Object} compass - Mongoose compass document
 ...
 */
async function refine(compass, userMessage, refinementMode = 'blueprint') {
  const client = getClient();
  if (!client) {
  ...
  const dossier = await buildUserDossier(compass.userId);
  const dossierBlock = formatDossierForPrompt(dossier);
  ...
  const refinements = compass.refinements || [];
  let historyBlock = '';
  ...
  const systemState = {
    name: compass.system.name,
    description: compass.system.description,
    icon: compass.system.icon,
    category: compass.system.category,
    phases: (compass.system.phases || []).map((p, pi) => ({
      index: pi,
      name: p.name,
  ...
  const userPrompt = `Original goal: "${compass.goal.text}"

REFINEMENT MODE: ${refinementMode}
${refinementMode === 'active' ? 'This compass is ACTIVE — habits are live. Patches will modify real habits the user is tracking. Be descriptive about concrete changes.\n' : ''}
...
Remember: Output ONLY valid JSON with "patches" and "assistantMessage" fields. No markdown, no explanation.`;
  ...
  try {
    ...
    console.error('[CompassEngine] Failed to parse refine response:', raw);
    throw new Error('AI returned an invalid response. Please try again.');
  }
...
/**
 * Apply patches to a compass's system in place.
  ...
  for (const patch of patches) {
    try {
      switch (patch.op) {
        ...
        default:
          console.warn('[CompassEngine] Unknown patch op:', patch.op);
      }
    } catch (err) {
      console.warn('[CompassEngine] Patch failed:', patch.op, err.message);
    }
  }
...
const CLARIFY_SYSTEM_PROMPT = `You are Bito's Compass Engine. Before generating a habit plan, you're assessing whether you have enough context to create a truly personalized plan.
...
For compound goals, you MUST acknowledge the structure and explain what compasses will be created.
...
If clarification is needed:
{
  ...
  "goalAnalysis": {
    ...
    "structureSummary": "string — if multi: '3 compasses: Fitness (goals 1,2), Learning (goal 3), Finance (goal 4)'. If single: null",
    "capacityNote": "string or null — e.g., 'You currently have 4 active habits. Adding 3 compasses is ambitious but doable if phased.' Only include if relevant."
  }
}
...
- For capacity advice: reference their current active habit count and active compass count. Be honest but not blocking — suggest sequencing if needed rather than refusing.`;
...
async function clarify(goalText, userId) {
  ...
  // Count active compasses for capacity context
  try {
    const Compass = mongoose.model('Compass');
    const activeCompassCount = await Compass.countDocuments({ userId, status: 'active' });
    if (activeCompassCount > 0) {
      promptParts.push(`\nUser currently has ${activeCompassCount} active compass(es).`);
    }
  } catch { /* non-critical */ }
  ...
  try {
    ...
    if (result.goalAnalysis) {
      ...
      result.goalAnalysis = {
        goalType: 'multi',
        structureSummary: `${parsed.suiteGroups?.length || parsed.subGoals?.length} compasses will be created for your ${parsed.subGoals?.length} goals.`,
        capacityNote: null,
      };
    }
    ...
    console.warn('[CompassEngine] Clarification failed, skipping:', err.message);

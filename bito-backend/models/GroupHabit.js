const mongoose = require('mongoose');

const groupHabitSchema = new mongoose.Schema({
  // Group reference
  groupId: {
    type: mongoose.Schema.Types.ObjectId,
    ref: 'Group',
    required: true,
    index: true
  },
  
  // Basic habit information (template for group)
  name: {
    type: String,
    required: [true, 'Habit name is required'],
    trim: true,
    maxlength: [100, 'Habit name cannot exceed 100 characters']
  },
  description: {
    type: String,
    trim: true,
    maxlength: [500, 'Description cannot exceed 500 characters']
  },
  
  // Created by (group member)
  createdBy: {
    type: mongoose.Schema.Types.ObjectId,
    ref: 'User',
    required: true
  },
  
  // Habit configuration
  category: {
    type: String,
    enum: ['health', 'productivity', 'learning', 'fitness', 'mindfulness', 'social', 'creative', 'other'],
    default: 'other'
  },
  color: {
    type: String,
    match: [/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/, 'Please provide a valid hex color'],
    default: '#3B82F6'
  },
  icon: {
    type: String,
    default: '🎯'
  },
  
  // Group-specific settings
  groupSettings: {
    isRequired: {
      type: Boolean,
      default: false  // Must all group members track this habit?
    },
    isTemplate: {
      type: Boolean,
      default: true   // Can members customize this habit?
    },
    allowCustomization: {
      type: Boolean,
      default: true   // Can members modify target, schedule, etc?
    },
    visibility: {
      type: String,
      enum: ['all', 'admins-only', 'creator-only'],
      default: 'all'
    },
    autoAssign: {
      type: Boolean,
      default: false  // Automatically add to new members' dashboards?
    }
  },
  
  // Default tracking settings (can be overridden by members)
  defaultSettings: {
    frequency: {
      type: String,
      enum: ['daily', 'weekly', 'monthly'],
      default: 'daily'
    },
    weeklyTarget: {
      type: Number,
      min: 1,
      max: 7,
      default: 3  // For weekly frequency: complete on any X days per week
    },
    target: {
      value: {
        type: Number,
        default: 1,
        min: 1
      },
      unit: {
        type: String,
        enum: ['times', 'minutes', 'hours', 'pages', 'miles', 'calories', 'glasses', 'custom'],
        default: 'times'
      },
      customUnit: {
        type: String,
        maxlength: 20
      }
    },
    schedule: {
      days: [{
        type: Number,
        min: 0,
        max: 6
      }], // 0 = Sunday, 1 = Monday, etc.
      reminderTime: {
        type: String,
        match: [/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/, 'Please provide time in HH:MM format']
      }
    }
  },
  
  // Adoption tracking
  adoptionStats: {
    totalAdopted: { type: Number, default: 0 },
    activelyTracking: { type: Number, default: 0 },
    completionRate: { type: Number, default: 0 },
    avgStreak: { type: Number, default: 0 }
  },
  
  // Status
  isActive: {
    type: Boolean,
    default: true
  },
  isArchived: {
    type: Boolean,
    default: false
  },
  
  // Metadata
  createdAt: {
    type: Date,
    default: Date.now
  },
  updatedAt: {
    type: Date,
    default: Date.now
  }
}, {
  timestamps: true,
  toJSON: {
    transform: function(doc, ret) {
      delete ret.__v;
      return ret;
    }
  }
});

// Indexes
groupHabitSchema.index({ groupId: 1, isActive: 1 });
groupHabitSchema.index({ createdBy: 1 });
groupHabitSchema.index({ category: 1 });

// Methods
groupHabitSchema.methods.canUserModify = function(userId, groupRole) {
  // Creator can always modify
  if (this.createdBy.toString() === userId.toString()) {
    return true;
  }
  
  // Admins and owners can modify if visibility allows
  if (['owner', 'admin'].includes(groupRole)) {
    return this.groupSettings.visibility !== 'creator-only';
  }
  
  return false;
};

groupHabitSchema.methods.canUserAdopt = function(userId, groupRole) {
  // Check visibility settings
  if (this.groupSettings.visibility === 'admins-only' && 
      !['owner', 'admin'].includes(groupRole)) {
    return false;
  }
  
  if (this.groupSettings.visibility === 'creator-only' && 
      this.createdBy.toString() !== userId.toString()) {
    return false;
  }
  
  return this.isActive && !this.isArchived;
};

// Static methods
groupHabitSchema.statics.findByGroup = function(groupId, userRole = 'member') {
  const visibilityFilter = {
    groupId,
    isActive: true
  };
  
  // Filter based on user role
  if (userRole === 'member') {
    visibilityFilter['groupSettings.visibility'] = { $in: ['all'] };
  } else if (userRole === 'admin') {
    visibilityFilter['groupSettings.visibility'] = { $in: ['all', 'admins-only'] };
  }
  // Owners can see everything
  
  return this.find(visibilityFilter)
    .populate('createdBy', 'name avatar')
    .sort({ createdAt: -1 });
};

groupHabitSchema.statics.getPopularHabits = function(groupId, limit = 10) {
  return this.find({
    groupId,
    isActive: true
  })
  .sort({ 'adoptionStats.totalAdopted': -1 })
  .limit(limit)
  .populate('createdBy', 'name avatar');
};

module.exports = mongoose.model('GroupHabit', groupHabitSchema);
